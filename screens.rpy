...
screen say(who, what):
    ...
    window:
        ...
        #if who is None:
            #background Image("gui/textbox_none.png", xalign=0.5, yalign=1.0)
    ...
    vbox:
        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                #This adds the Inventory screen to the sidebar menu.
                textbutton _("Inventory") action [ShowMenu("inventory"), SensitiveIf(renpy.get_screen("inventory") == None)]
...
screen inventory():
    ...
    use game_menu("Inventory"):
        ...
        fixed:
            ...
            ## The grid of item slots.
            grid gui.item_slot_cols gui.item_slot_rows:
                style_prefix "slot"
                ...
                for i in range(gui.item_slot_cols * gui.item_slot_rows):

                    $ slot = i + 1

                    button:
                        #Only make a button if the item has been acquired.
                        if item_acquired(slot):
                            #This controls what the item actually does when used. 
                            #Implementation will likely vary. Probably would need to set up timing in your script for item usage.
                            #Nothing says the items have to have a use either. Could just be a visual aid, etc..
                            
                            #Currently, Professor Monika reminds us of the proper times for item usage.
                            action Show(screen="dialog", message="Monika's words echoed:\n[player]! This isn't the time to use that.", ok_action=Hide("dialog"))
                            has vbox
                            #Only display the item's image if the item is marked as acquired.
                            #I went with items/Item#.png for pictures of the items. Change this to pull item pictures from different paths in your images.rpa file.
                            add "/items/Item" + str(slot) + ".png" xalign 0.5
                            #To-do: Item descriptions here
                            #text item_description(slot): 
                            #    style "slot_button_text"
                            #    size 30
                                
                        else:
                            add "/gui/button/quick_idle_background.png"
            ...
            hbox:
                ...
                # range(1, 2) gives one page of items.
                # Adjust this to add more pages for more items.
...
init python:
    ...
    def item_acquired(itemno):
    #This function lets us evaluate the item's state by passing it the slot number.
    #Add more lines here if you want more items.
    #There is probably a much better way of doing this. I am not good with computer.
        if itemno == 1:
            return item1_acquired
        elif itemno == 2:
            return item2_acquired
        elif itemno == 3:
            return item3_acquired
        elif itemno == 4:
            return item4_acquired
        else:
            return False
    def item_description(itemno):
    #This function pulls the item descriptions onto the inventory screen.
