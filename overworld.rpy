    ...
    class OverworldDisplayable(renpy.Displayable):
        ...
        class Player():
            ...
            def collide(self, walls):
                ...
                for _block in filter(lambda block: block.__class__ != OverworldDisplayable.Portal, block_hit_list): #check the objects we hit
                ...
                for _block in filter(lambda block: block.__class__ != OverworldDisplayable.Portal, block_hit_list):
                    if self.yvel > 0:
        ...
        def __init__(self, map_layout = None, tileList = None, portals = None, portal_tiles = None, enemy_layout =None, enemySprites = None, NPCSprites = None, groundLayout = None, playerSprites = None, playerX = None, playerY = None, scrolling = True):
            ...
            self.bakedGround = None
            self.gBakedSize = None
        ...
        def bakeGround(self, width, height, st, at):
            if self.bakedGround is None or self.gBakedSize != renpy.get_physical_size():
                self.bakedGround = renpy.Render(width, height)
                self.gBakedSize = renpy.get_physical_size()
                for g in self.groundList: #ground is rendered first, with no sorting.
                    self.bakedGround.blit(renpy.render(g.image, g.rect.width, g.rect.height, st, at), (g.rect.x, g.rect.y))
            return self.bakedGround
        ...
        def render(self, width, height, st, at):
            ...
            map_child.blit(self.bakeGround(width,height, st, at),(0,0))

            for zl in filter(lambda zl: zl.zLayer == 2, self.zList): #layer 2 has pretty much everything else
                map_child.blit(renpy.render(zl.image, zl.rect.width, zl.rect.height, st, at), (zl.rect.x, zl.rect.y))

            for zl in filter(lambda zl: zl.zLayer == 3, self.zList): #and finally, higher altitude items like roofs
                map_child.blit(renpy.render(zl.image, zl.rect.width, zl.rect.height, st, at), (zl.rect.x, zl.rect.y))
            ...
            for n in filter(lambda npc: not npc.talking, self.npcList):
                    n.update(self.NPCObstacles, self.player)

            for s in filter(lambda s: s.goToLabel is not None, self.buildings): #handle collision with important buildings
            ...
            for p in filter(lambda p: self.player.rect.colliderect(p.rect), self.portalList):
